...
{
    public sealed class MethodDurationByChange : IMethod
    {
        private readonly Services.Rtdbs.IStatisticalReader reader;

        public MethodDurationByChange(AggregatedAnalogValue avalue)
        {
            reader = avalue.Core.Rtdb.CreateDurationStatisticalReader();
        }

        public MethodTypeByChange GetMethodTypeByChange()
        {
            return MethodTypeByChange.duration;
        }

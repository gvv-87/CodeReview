...
{
    public sealed class MethodDurationOffByChange : IMethod
    {
        private readonly Services.Rtdbs.IStatisticalReader reader;

        public MethodDurationOffByChange(AggregatedAnalogValue avalue)
        {
            reader = avalue.Core.Rtdb.CreateDurationOffStatisticalReader();
        }

        public MethodTypeByChange GetMethodTypeByChange()
        {
            return MethodTypeByChange.durationOff;
        }

...
{
    public sealed class MethodIntegByChange : IMethod
    {
        private readonly Services.Rtdbs.IStatisticalReader reader;

        public MethodIntegByChange(AggregatedAnalogValue avalue)
        {
            reader = avalue.Core.Rtdb.CreateIntegStatisticalReader();
        }

        public MethodTypeByChange GetMethodTypeByChange()
        {
            return MethodTypeByChange.integ;
        }
        ...
        public void Write(AggregatedAnalogValue avalue, Calc.ICalcData data, DateTime from, DateTime to)
        {
            ...
            avalue.Write(data, sum * 10, m_quality, avalue.GetResultWriteTime(to));
        }

        public override string ToString()
        {
            return "integByChange";
        }
    }
}

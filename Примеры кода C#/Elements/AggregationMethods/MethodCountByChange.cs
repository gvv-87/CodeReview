...
{
    public sealed class MethodCountByChange : IMethod
    {
        private readonly Services.Rtdbs.IStatisticalReader reader;

        public MethodCountByChange(AggregatedAnalogValue avalue)
        {
            reader = avalue.Core.Rtdb.CreateCountStatisticalReader();
        }

        public MethodTypeByChange GetMethodTypeByChange()
        {
            return MethodTypeByChange.count;
        }

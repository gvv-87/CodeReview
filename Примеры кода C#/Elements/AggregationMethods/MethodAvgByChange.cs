...
{
    public sealed class MethodAvgByChange : IMethod
    {
        private readonly Services.Rtdbs.IStatisticalReader reader;

        public MethodAvgByChange(AggregatedAnalogValue avalue)
        {
            reader = avalue.Core.Rtdb.CreateAvgStatisticalReader();
        }

        public MethodTypeByChange GetMethodTypeByChange()
        {
            return MethodTypeByChange.avg;
        }
        ...
        public void Write(AggregatedAnalogValue avalue, Calc.ICalcData data, DateTime from, DateTime to)
        {
            ...
            avalue.Write(data, sum / count, m_quality, avalue.GetResultWriteTime(to));
        }
